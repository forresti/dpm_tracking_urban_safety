% this code is adapted from voc-release5/vis/showboxes.m
% showboxes_forTracking adds labels for detection numbers on bounding boxes.
function showboxes_forTracking(im, boxes, out)
...
%      s = '--';
    ...
    if(length(x1)>0)
        for boxIdx=1:length(x1) %Forrest
            text(double(x1(boxIdx)), double(y1(boxIdx)), int2str(boxIdx), 'color', c, 'BackgroundColor', 'black'); %label detection ID
        end
    end
  end
end
